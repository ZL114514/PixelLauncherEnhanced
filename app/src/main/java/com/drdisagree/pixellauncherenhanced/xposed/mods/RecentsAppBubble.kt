...
class RecentsAppBubble(context: Context) : ModPack(context) {
    ...
    override fun handleLoadPackage(loadPackageParam: LoadPackageParam) {
        ...
        appInfoShortcutClass
            ...
            .runBefore { param ->
                ...
                log(this@RecentsAppBubble, "setIconAndLabelFor hit, icon=${iconView::class.java.simpleName}, label=$bubbleLabel")
                ...
                applyShortcutLabel(param.thisObject, bubbleLabel, "setIconAndLabelFor")
                param.result = null
            }
        ...
        appInfoShortcutClass
            ...
            .runBefore { param ->
                ...
                applyShortcutLabel(param.thisObject, bubbleLabel, "setIconAndContentDescriptionFor")
                log(this@RecentsAppBubble, "setIconAndContentDescriptionFor hit, label=$bubbleLabel")
                param.result = null
            }
        ...
        if (labelMethodName != null) {
            appInfoShortcutClass
                ...
                .runBefore { param ->
                    ...
                    val context = try {
                        (param.thisObject.getField("mOriginalView") as? View)?.context ?: mContext
                    } catch (throwable: Throwable) {
                        log(this@RecentsAppBubble, "labelGetter: mOriginalView unavailable: ${throwable.message}")
                        mContext
                    }
        ...
        appInfoShortcutClass
            ...
            .runBefore { param ->
                ...
                try {
                    param.thisObject.callMethod("dismissTaskMenuView")
                } catch (throwable: Throwable) {
                    log(this@RecentsAppBubble, "onClick: dismissTaskMenuView failed: ${throwable.message}")
                }
    ...
    private fun handlePopupProxyMethod(
        ...
        when (methodName) {
            "onClick" -> {
                val itemInfo = try {
                    shortcut.getField("mItemInfo")
                } catch (throwable: Throwable) {
                    log(this, "popup onClick: missing mItemInfo: ${throwable.message}")
                    null
                }
                val target = try {
                    shortcut.getField("mTarget")
                } catch (throwable: Throwable) {
                    log(this, "popup onClick: missing mTarget: ${throwable.message}")
                    null
                }
                showBubbleFromPopup(target, itemInfo)
                try {
                    shortcut.callMethod("dismissTaskMenuView", target)
                } catch (throwable: Throwable) {
                    log(this, "popup onClick: dismissTaskMenuView failed: ${throwable.message}")
                }
                return null
            }
            ...
            "setIconAndLabelFor" -> {
                ...
                val bubbleLabel = getBubbleLabel(iconView.context)
                labelView.text = bubbleLabel
                applyShortcutLabel(shortcut, bubbleLabel, "popup setIconAndLabelFor")
                return null
            }
    ...
    private fun showBubbleFromPopup(target: Any?, itemInfo: Any?) {
        val intent = try {
            (itemInfo?.callMethod("getIntent") as? Intent)?.let(::Intent)
        } catch (throwable: Throwable) {
            log(this, "showBubbleFromPopup: getIntent failed: ${throwable.message}")
            null
        } ?: return

        val user = try {
            (itemInfo?.getField("user") as? UserHandle) ?: Process.myUserHandle()
        } catch (throwable: Throwable) {
            log(this, "showBubbleFromPopup: user field failed: ${throwable.message}")
        ...
        log(this, "showBubbleFromPopup: fallback to SystemUiProxy")
    ...
    private fun applyShortcutLabel(shortcut: Any, label: CharSequence, source: String) {
        val fields = shortcut::class.java.declaredFields.map { it.name }.toSet()

        when {
            shortcut.hasMethod("setLabel", CharSequence::class.java) -> {
                try {
                    shortcut.callMethod("setLabel", label)
                    return
                } catch (throwable: Throwable) {
                    log(this, "$source: setLabel failed: ${throwable.message}")
                }
            }

            "mLabel" in fields -> {
                try {
                    shortcut.setField("mLabel", label)
                    return
                } catch (throwable: Throwable) {
                    log(this, "$source: mLabel set failed: ${throwable.message}")
                }
            }

            "label" in fields -> {
                try {
                    shortcut.setField("label", label)
                    return
                } catch (throwable: Throwable) {
                    log(this, "$source: label set failed: ${throwable.message}")
                }
            }
        }
    }
